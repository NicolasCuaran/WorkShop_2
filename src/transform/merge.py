...
def merge_data(spotify_df: Union[pd.DataFrame, str], grammys_df: Union[pd.DataFrame, str], spotify_api_df: Union[pd.DataFrame, str] = None) -> Optional[str]:
    ...
    try:
        ...
        fill_columns_api = ["country", "begin_date", "end_date", "type", "gender", "disambiguation"]
        fill_null_values(df_merged, fill_columns_api, "Not available")

        columns_drop = [
            "year", "artist", "nominee", "nominee_clean", "artist_name", "artist_id",
            "nominee_clean", "track_name_clean", "artist_name_clean"
        ]
